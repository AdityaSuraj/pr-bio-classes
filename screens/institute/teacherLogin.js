...
import { instituteRootlink} from '../../constants/link';
...
const TeacherLogin = ()=>{
    ...
    React.useEffect(()=>{
        getPlainValueFromStorage('token').then((res)=>{
            if(res !== null){
    ...
    function handlePhoneRegistration(){
        if(email.length === 0){
            ...
            setIsLoading(true);
            fetch(`${instituteRootlink}/login`,{method:"POST",
            headers:{
            ...
            .then((json)=>{
                ...
                if(json['message'] === "success"){
                    saveInStorage('token',json['token']);
                    navigation.replace('TeacherTabNavigation')
                }else{
                    alert(json['message']);
                }
            })
    ...
    return (
        <View style={{flex:1,backgroundColor:"rgba(255,165,0,0.05)",position:"relative"}}>
            <View style={{margin:10,borderRadius:5,marginTop:40,backgroundColor:"white",alignItems:"center",padding:16}}>
                ...
                <Text style={{fontSize:20,fontWeight:"bold"}}>Teacher Login</Text>
